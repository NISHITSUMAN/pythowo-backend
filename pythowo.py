...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("(・`ω´・) You didn't gib me a code fiwe to wun... How couwd u >w<")
        print("Usage: python pythowo.py <youwa-code.pyowo>")
        sys.exit(1)

    code_path = sys.argv[1]

    if not os.path.exists(code_path):
        print(f"(>w<) Oopsie! I can't find dat fiwe: '{code_path}'")
        print("Pwease check da path again... maybe typo? 👉👈")
        sys.exit(1)

    try:
        with open(code_path, "r") as fiwe:
            cwode = fiwe.read()

        result, error = run(code_path, cwode)

        if error:
            print("(╥﹏╥) Uh oh! I bwoke...")
            print(f"✧･ﾟ: {error.as_string()}")
            sys.exit(1)
        else:
            print(result)
            sys.exit(0)
        ...
        print("(；ω；) I twied weally hawd but somethin went wong~")
        print(f"✧･ﾟ: {str(e)}")
        sys.exit(1)
